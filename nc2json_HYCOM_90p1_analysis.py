...
#Start_Date, End_Date = np.datetime64('now','h') + np.timedelta64(1,'h'), np.datetime64('now','h') + np.timedelta64(1,'h')
...
link = link + '?'\
       ...
       + 'water_u[{:d}][0][0:{:d}:345][0:{:d}:540],'.format(tid[0],slice_index,slice_index)\
       + 'water_v[{:d}][0][0:{:d}:345][0:{:d}:540]'.format(tid[0],slice_index,slice_index)

import importlib.util #-- python original package, to check whether xarray is installed.
package_name = 'xarray'
...
if spec is None:
	...
	u0 = np.squeeze(np.ma.masked_invalid(nc.variables['water_u'][:]).filled(fill_value=0))[::-1]
	v0 = np.squeeze(np.ma.masked_invalid(nc.variables['water_v'][:]).filled(fill_value=0))[::-1]
else:
	print('run on Xarray')
	import xarray as xr
	...
	nx, ny = len(lon), len(lat)
	u0 = df.fillna(0).water_u.data[::-1]
	v0 = df.fillna(0).water_v.data[::-1]
	ref_time = str(df.time.data)
